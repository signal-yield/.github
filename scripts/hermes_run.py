...
def call_openai(prompt: str) -> str:
    ...
    try:
        ...
        # Some accounts/models may not have web_search enabled. Fall back to model-only
        # generation so the workflow still produces a usable research brief.
        response = client.responses.create(
            model=model,
            input=(
                prompt
    ...
    # Conservative fallback for SDK response shapes.
    chunks: list[str] = []
